create table user_friend_names_model(USER char, FRIEND char);
insert into user_friend_names_model(USER, FRIEND)

SELECT M.USER as USER_NAME, S2.USER as FRIEND_NAME
...
*/
/*
create table modelbandweeks_strict_adopt(USER char, week_ID integer, COUNTmod integer, BAND_ID integer, WEEK_USER integer, WEEK_BAND integer);
insert into modelbandweeks_strict_adopt(USER, week_ID, COUNTmod, BAND_ID, WEEK_USER, WEEK_BAND)

select * from modelbandweeks_joint
where WEEK_USER <= WEEK_BAND
*/
/*
create table member_adoption_per_band_all_users_data(member_adopt integer, BAND_ID integer);
insert into member_adoption_per_band_all_users_data(member_adopt, BAND_ID)

select count(distinct USER) as member_adopt, BAND_ID from 
(SELECT * FROM modelbandweeks WHERE USER in (select USER from dbr.member_list_model))
group by BAND_ID
*/
/*
select count(distinct USER) as member_adopt, BAND_ID from adoptobs
group by BAND_ID;
*/
--select sum(adopt) from adopt_strict_trends_left_join_full_weeks

/*
create table member_list_model(USER_ID integer, USER char);
insert into member_list_model(USER_ID, USER)

SELECT USER_ID, USER FROM dbaf.userlist
WHERE USER_ID in 
(select distinct USER from adoptobs)
*/
--select count(distinct BAND_ID) from adopt_all_users_2588
--select count(*) from modelbandweeks_joint
--select sum(adopt) from adopt_strict_all_users_2588
...
/*
create table adopt_strict_all_users_2588(BAND_ID integer, WEEK_ID integer, adopt integer);
insert into adopt_strict_all_users_2588(BAND_ID, WEEK_ID, adopt)

SELECT BAND_ID, week as WEEK_ID, count(distinct USER) as adopt 
FROM (select USER, BAND_ID, min(week_ID) as week
from dbas.modelbandweeks_strict_adopt
