...
db_mod_adoptions_strict <- tbl(db, "mod_adoptions_lenient")
db_peak_strict <- tbl(db, "peak_lenient")

# for computing EA score
adoptingweek_lenient <- tbl(db,sql("select USER_ID, mod_adoptions_lenient.BAND_ID, mod_adoptions_lenient.week_mod, introdate.week_mod as bandintro, peak_lenient.week_mod as bandpeak
from mod_adoptions_lenient, peak_lenient, introdate
where mod_adoptions_lenient.BAND_ID = peak_lenient.BAND_ID and mod_adoptions_lenient.BAND_ID = introdate.BAND_ID
           "))

# adoptweek <- collect(adoptingweek)
adoptweek <- collect(adoptingweek_lenient, n = Inf)

write.table(adoptweek, "adoptingweek_lenient.csv", row.names=F, col.names=F, sep=",")
