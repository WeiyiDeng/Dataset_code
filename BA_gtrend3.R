...
trydata <- read.csv(file="E:/Trans/Transfer from old Toshiba/FileProcess/bands2588_ori/4_ori.csv", 
                    header=TRUE, sep=",")
...
trydata <- read.csv(file="E:/Trans/Transfer from old Toshiba/FileProcess/bands2588_ori/export_list.csv", header=FALSE, sep=",")
# trydata
...
for (i in 1:2588) {
  if (i %in% except_list){
    ...
    print(i)
    path = paste("E:/Trans/Transfer from old Toshiba/FileProcess/bands2588_ori/", i, "_ori.csv", sep = "")
...
write.csv(file="gtrends_ori.csv", x=gtrends_mat)
...
write.csv(file="gtrends_ori_423.csv", x=gtrends_mod)
